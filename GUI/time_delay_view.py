...
class TimeDelayView(QDialog):
    ...
    def show_plot(self, cross_corr, shift_in_samples, sampling_time):
        t = cross_corr[0]
        cc = cross_corr[1]
        self.ui.mpl_widget.canvas.ax.bar(t, cc)
        self.ui.mpl_widget.canvas.ax.set_ylim(min(cc) - 0.1 * (max(cc) - min(cc)), max(cc) + 0.1 * (max(cc) - min(cc)))
        self.ui.mpl_widget.canvas.ax.axvline(shift_in_samples, color='red')
    ...
    def display_shift_in_samples(self, shift_in_samples):
        self.ui.shiftSamplesLineEdit.setText(f"{shift_in_samples:.2f}")
